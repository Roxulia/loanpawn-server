...
class TenantCustomerService extends BaseTenantService
{
    ...
    public function list(int $perPage = 15, ?string $search = null, bool $showUnknownCustomer = false): TenantCustomerListPage
    {
        $this->permissionService->authorizeCustomerList();
        $search = $this->normalizeSearch($search);
        $today = CarbonImmutable::today();
        $paginator = $this->repository->paginate($perPage, $search, $showUnknownCustomer);
        ...
        $summary = $this->repository->customerListSummary($today, self::RISK_TRUST_SCORE_THRESHOLD, $showUnknownCustomer);
    ...
    ): TenantCustomerUpsertResult
    {
        ...
        // Restoration of an existing shared identity without creating duplicates
        $tenantId = app(\App\Support\TenantContext::class)->id();
        $person = $this->repository->findPersonForIdentity($tenantId, $request->email, $request->phone, $request->nrc);
        if ($person !== null) {
            $existingCustomer = $this->repository->customerForPerson($person->id);
            if ($existingCustomer !== null && ! $existingCustomer->trashed()) {
                if ($reuseActiveCustomer) return TenantCustomerUpsertResult::existing(TenantCustomerDetail::fromModel($existingCustomer));
                throw new DuplicateValueFound(null);
            }
            if ($existingCustomer !== null) {
                $restored = DB::transaction(function () use ($existingCustomer, $person) {
                    $customer = $this->repository->restoreCustomer($existingCustomer);
                    if ($lender = $this->repository->lenderForPerson($person->id)) {
                        if ($lender->trashed()) $this->repository->restoreLender($lender);
                    } else {
                        $this->repository->createLender(['tenant_id' => $person->tenant_id, 'person_id' => $person->id, 'code' => $this->tableIdGenerationService->generate('tenant_lenders', CarbonImmutable::now()), 'created_by' => $this->resolveCurrentTenantUserId()]);
                    }
                    return $customer;
                });
                $this->flushTenantCustomerListCache();
                return TenantCustomerUpsertResult::created(TenantCustomerDetail::fromModel($restored));
            }
        }
        ...
        $customer = DB::transaction(function () use ($request) {
            // Creation of the authoritative shared identity
            ...
            $customer = $this->repository->create([
                ...
                'created_by' => $request->createdBy,
                'is_deleted' => false,
                'is_auto_generated' => $request->isAutoGenerated,
            ]);

            // Automatic creation of the matching lender profile
            $this->repository->createLender([
                'tenant_id' => app(\App\Support\TenantContext::class)->id(),
                'person_id' => $person->id,
                'code' => $this->tableIdGenerationService->generate('tenant_lenders', CarbonImmutable::now()),
                'created_by' => $request->createdBy,
            ]);
    ...
    public function update(TenantCustomerUpdate $request): TenantCustomerDetail
    {
        ...
        $updatedCustomer = DB::transaction(function () use ($customer, $data, $original) {
            ...
            // Synchronization of the shared person before legacy customer columns
            if ($customer->person !== null) {
                $personData = array_intersect_key($data, array_flip(['name', 'nrc', 'email', 'phone', 'address', 'note']));
                if ($personData !== []) {
                    $personData['update_key'] = $customer->person->update_key + 1;
                    $this->repository->updatePerson($customer->person, $personData);
                }
            }
