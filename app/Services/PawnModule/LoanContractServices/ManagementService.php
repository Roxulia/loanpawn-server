...
class ManagementService extends BaseTenantService
{
    ...
    public function create(LoanContractSlipCreate $request): LoanContractSlipDetail
    {
        ...
        if (! $this->tenantSettingService->currentTenantRequiresLoanSlipCustomerInfo()) {
            $request->customer = new TenantCustomerCreate(
    ...
    protected function validateCreateRequest(LoanContractSlipCreate $request): void
    {
        ...
        $customerNrc = $this->normalizeNullableString($request->customer->nrc);

        $request->customer->email = $customerEmail;
        $request->customer->phone = $customerPhone;
        $request->customer->nrc = $customerNrc;

        if ($this->tenantSettingService->currentTenantRequiresLoanSlipCustomerInfo() && trim($request->customer->name) === '') {
            throw new InvalidTenantRequest('Customer name is required.');
        }

        if ($this->tenantSettingService->currentTenantRequiresLoanSlipCustomerInfo() && $customerNrc === null && $customerEmail === null && $customerPhone === null) {
            throw new InvalidTenantRequest('One of NRC, email, or phone is required.');
        }
