...
<div class="content-stack admin-currency-page">
    ...
    <section class="content-card admin-finance-create-card">
        <div class="admin-section-heading"><div><p class="admin-section-kicker">Catalog setup</p><h2>Add currency</h2><p>Define formatting and rounding defaults available to tenants.</p></div></div>
        <form method="POST" action="{{ route('admin.currencies.store') }}" class="filter-grid">@csrf
            ...
            <label>Decimals<input name="decimal_precision" type="number" min="0" max="8" value="{{ old('decimal_precision', 2) }}" required></label>
            <label>Rounding<select name="rounding_mode">@foreach(['HALF_UP','HALF_DOWN','HALF_EVEN','UP','DOWN'] as $mode)<option @selected(old('rounding_mode', 'HALF_UP') === $mode)>{{ $mode }}</option>@endforeach</select></label>
            <label>Adjustment step<input name="adjustment_step" type="number" min="0" step="0.00000001" value="{{ old('adjustment_step') }}"></label>
