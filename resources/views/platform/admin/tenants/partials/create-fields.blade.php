...
<div class="admin-tenant-create__grid">
    ...
    <label>Subdomain<input name="subdomain" value="{{ old('subdomain') }}" maxlength="63" pattern="[A-Za-z0-9_-]+"></label>
    ...
    <label>Plan<select name="plan_id" required>@foreach ($categories as $category)@foreach ($category->packages as $plan)<option value="{{ $plan->id }}" data-category-id="{{ $category->id }}" @selected((string) old('plan_id') === (string) $plan->id)>{{ $plan->name }}</option>@endforeach @endforeach</select></label>
